...
def ResolvedGeom(problem,show_plots=False, r_eval=[],n_ref_max=5 ):
    ...
    for order in orders:
        ...
        for n_ref in range(n_refs):
            ...
            name_str_eval = "3D-convex-comp-unfitted-stab-u-eval" + "-" + "reflvl{0}".format(n_ref)+"-p{0}".format(order)+"-mus({0},{1})".format(int(mu[0]),int(mu[1]))+"-ks({0},{1})".format(int(k[0]),int(k[1]))+".dat" 
        ...
        name_str = "3D-convex-comp-unfitted-stab" + "-"  + "-p{0}".format(order)+"-q{0}".format(order_geom)+"-mus({0},{1})".format(int(mu[0]),int(mu[1]))+"-ks({0},{1})".format(int(k[0]),int(k[1]))+".dat" 
...
domain_type = "convex-3D-concentric"
...
#hlist = [0.4,0.3,0.2,0.1]
#hlist = [0.1,0.09,0.08,0.07,0.065,0.06]
#hlist = [0.09]
#hlist = [0.1,0.09,0.08,0.07]
#n_ref_max = len(hlist)
...
ResolvedGeom(problem=helmholtz_3D_ball ,show_plots=True, r_eval=r_eval, n_ref_max=3)

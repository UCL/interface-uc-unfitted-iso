...
def SolveZNoCut(problem, order = 1, n_refs = 0, order_geom = 1, theta_perturb = None, order_dual = 1, stabi_dict = None, geom_stab_all_el = True, vtk_output = False,adjoint_consistent=True,
    ...
    elif problem.domain_type in ["convex", "data-half", "convex-3D"]:
    ...
    sol_sqr = sum([(solution[i])**2 * dCut(lsetp1, dt,definedon=mesh.Materials("only_B|omega"), deformation=lsetadap.deform, order=2*order) for i,dt in zip([0, 1],[NEG,POS] ) ])
    sol_norm = sqrt(Integrate(sol_sqr, mesh))
    grad_sqr = sum([ InnerProduct(sol_gradient[i], sol_gradient[i]) * dCut(lsetp1, dt,definedon=mesh.Materials("only_B|omega"), deformation=lsetadap.deform, order=2*order) for i,dt in zip([0, 1],[NEG,POS] ) ])
    grad_norm = sqrt(Integrate(grad_sqr, mesh))

    err_sqr = (gfuh[0]   - solution[0])**2 * dCut(lsetp1, NEG,definedon=mesh.Materials("only_B|omega"), deformation=lsetadap.deform, order=2*order)
    err_sqr += (gfuh[1]  - solution[1])**2 * dCut(lsetp1, POS ,definedon=mesh.Materials("only_B|omega"), deformation=lsetadap.deform, order=2*order)
    l2_err = sqrt(Integrate(err_sqr, mesh))
    rel_err = l2_err / sol_norm

    grad_err_sqr = InnerProduct(grad(gfuh[0]) - sol_gradient[0], grad(gfuh[0]) - sol_gradient[0] ) * dCut(lsetp1, NEG,definedon=mesh.Materials("only_B|omega"), deformation=lsetadap.deform, order=2*order)
    grad_err_sqr += InnerProduct(grad(gfuh[1]) - sol_gradient[1], grad(gfuh[1]) - sol_gradient[1]) * dCut(lsetp1, POS,definedon=mesh.Materials("only_B|omega"), deformation=lsetadap.deform, order=2*order)

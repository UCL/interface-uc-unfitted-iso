...
def ResolvedGeom(problem,show_plots=False, r_eval=[]):
    ...
    for order in orders:
        ...
        for n_ref in range(n_refs):
            ...
            name_str_eval = "3D-comp-unfitted-stab-u-eval" + "-" + "reflvl{0}".format(n_ref)+"-p{0}".format(order)+"-mus({0},{1})".format(int(mu[0]),int(mu[1]))+"-ks({0},{1})".format(int(k[0]),int(k[1]))+".dat" 
